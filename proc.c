...
struct proc* get_ptable_proc(int i){
      ...
      if(p->state == ZOMBIE){
        ...
        for(int i = 0; i < NPDENTRIES; ++i){
          ...
          for(int j=0;j<NPTENTRIES;++j){
              if(!(pt[j] & PTE_P) && (pt[j] & PTE_SP)){
                  uint physical_address = ((pt[j]) >> 12);
                  uint slot_idx = (physical_address - 2)/ 8;
                  clear_swap_slot(slot_idx,&pt[j]);
...
struct proc* select_victim_process(){
  struct proc* p;
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    if ((p->rss > max_rss) || (p->rss == max_rss && p->pid < pid)){
      max_rss = p->rss;
      pid = p->pid;
      victim = p;
    }
